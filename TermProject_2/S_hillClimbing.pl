...
:-dynamic(uphill_mv/3). /* id,state,h_value(store up hillmoves)*/
:-dynamic(up_id/1).
:-dynamic(id/1).
:-dynamic(max_val/1).
:-dynamic(threshold/1).
...
gnrt_sucsr(L):- assert(id(1)), assert(state(1,'c',L,50)),
	assert(up_id(1)),incr_id, mk_new(1,L), retract(id(_)), evaluate.

incr_id:-id(V), V1 is V+1, retract(id(_)), assert(id(V1)).
incr_up_id:-up_id(V), V1 is V+1, retract(up_id(_)), assert(up_id(V1)).
...
checkall:- state(_,'c',_,V1), best(I,V2) ,V2>V1,state(I,_,L,_),retractall(state(_,_,_,_)),
	write_list(['\nUp Hill moves : ',V2]),
	retractall(uphill_mv(_,_,_)),up_id(Z),assert(uphill_mv(Z,L,V2)),incr_up_id,
	gnrt_sucsr(L),!.
checkall:- rndm_upHill,!.
...
rndm_upHill:-up_id(V), X is random(V)+1,uphill_mv(X,L,_),
